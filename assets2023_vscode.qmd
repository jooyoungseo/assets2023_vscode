...
For example, with VoiceOver on, the terminal buffer is entered for a task terminal. A line with an error is focused and read by VoiceOver.

![VoiceOver reads "rvice.ts(2018,13: Cannot find name 'taskSet'"](images/terminal-accessible-buffer.png)
...
For example, the terminal accessibility help menu contains helpful information for screen reader users such as commands to run. A screen reader user can use arrow keys to read the content line by line, character by character.

![A line in the help menu is focused and VoiceOver reads "The terminal has a feature called shell integration that offers an enhanced experience and provides useful commands for screen readers such as:"](/images/terminal-help-menu.png)
